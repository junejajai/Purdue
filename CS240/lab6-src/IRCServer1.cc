...
void IRCServer::createRoom(int fd, const char * user, const char * password, const char * args)
{
	...
	for(int i = 0;i<nr;i++)
	{	printf("strlen args %d strlen rooms %d\n",strlen(args),strlen(rooms[i].roomname));
		printf("rooms are %s\n",rooms[i].roomname);
		int x = strcmp(args,rooms[i].roomname) + 13;
		if(!strcmp(args,rooms[i].roomname)||x==0)
	...
	//char * temp = args;
	rooms[nr].roomname = (char*)malloc(20*sizeof(char));
	rooms[nr].roomname = strdup(args);
	printf("args is %s room is %sfgd\n",args,rooms[nr].roomname);
	nr++;
	const char * hs = "OK\r\n";
	write(fd,hs,strlen(hs));
	//printf("args %s rooms[i].n %sfgh\n",args,rooms[nr-1].roomname);
	return;
...
IRCServer::leaveRoom(int fd, const char * user, const char * password, const char * args)
{
	...
	{
	int x = strcmp(rooms[bn].u.at(k),user)+13;
	if(!strcmp(rooms[bn].u.at(k),user)||x==0)
	{
	flag1 = 1;
	break;
...
IRCServer::getMessages(int fd, const char * user, const char * password, const char * args)
{
	...
	{
		printf("str is %d\n",strcmp(args,rooms[i].roomname));
		printf("args %s room %sfddd\n",args,rooms[i].roomname);	
		printf("leng of args is %d and leng of rrom is %d\n",strlen(args1),strlen(rooms[i].roomname));			
		int x = strcmp(rname,rooms[i].roomname) + 13;
		//int x2 = strcmp(args,rooms[i].roomname) + 56;
		char * r = (char*)malloc(50*sizeof(char));
		strcpy(r,rname);
		strcat(r,"\r\n");		
				printf("str is %d\n",strcmp(rname,rooms[i].roomname));
				int x2 = strcmp(rname,rooms[i].roomname) + 1;
		if(!strcmp(rname,rooms[i].roomname)||x==0||x2==0)
		{
			flag = 1;
			bn = i;
			break;
	...
	{
	printf("u %s ur %s str %d\n",user,rooms[bn].u.at(huk),strcmp(rooms[bn].u.at(huk),user));
